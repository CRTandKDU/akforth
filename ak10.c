...
static void f_immediate(void){ latest->immediate=1; }
...
static void f_1branch(void) { // jump if top of stack is not zero
...
static void f_logand(void){
  // Use stack for internal storage
  *code++ = xt_dup;
  *code++ = xt_1branch;
  sp_push((cell_t)code++);	// push forward reference on stack
  *code++ = xt_leave;
...
static void f_tick(void) {
  ...
  if(xt) sp_push((cell_t)xt);
      ...
      char errbuf[128];
      sprintf( errbuf, "WU Error: %s", w);
      terminate( errbuf );		// word unknown
    }
}
...
static void f_putbyte(void){
  cell_t *addr = (cell_t *) sp_pop();
  cell_t val  = (cell_t) sp_pop();
  *addr = GET_LO_CHAR(val);
}
...
static void f_xt_to_name(void) {*sp=(cell_t)((xt_t*)*sp)->name;}

static void f_xt_to_data(void) {*sp=(cell_t)((xt_t*)*sp)->data;}

static void f_xt_to_clientdata(void) {*sp=(cell_t)((xt_t*)*sp)->clientdata;}

static void f_xt_set_clientdata(void){xt_t *xt = (xt_t *)sp_pop();xt->clientdata = (cell_t *) sp_pop();}
...
// EXTENSIONS
static void f_alloc(void){
  int size = (int) sp_pop();
  sp_push( (cell_t) MALLOC( (unsigned long long) (size) ) );
}
...
static void register_primitives(void) {
  ...
  latest->has_lit=1;
  add_word( "immediate", f_immediate );
  latest->immediate=1;
  // Memory load/store
  add_word("!",		f_put);
  ...
  latest->immediate=1;
  add_word("logand",	f_logand);
  latest->immediate=1;
  add_word("logor",	f_logor);
  latest->immediate=1;
